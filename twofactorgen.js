...
vectorWatch.on('config', function(event, response) {
    ...
    var secret = response.createAutocomplete('Secret');
    secret.setHint('Please enter your base32 encoded secret.');
    secret.setDynamic(true);
    secret.setAsYouType(1);
    response.send();
});
...
vectorWatch.on('subscribe', function(event, response) {
    ...
    var key = event.getUserSettings().settings['Secret'].name.split(' ').join(''); //remove spaces

    response.setValue(getOtp(key));
    response.send();
});
...
vectorWatch.on("schedule", function(records){
    records.forEach(function(record) {
        var key = record.userSettings['Secret'].name;
        record.pushUpdate(getOtp(key));
    });
});
//based on the jsfiddle http://jsfiddle.net/russau/ch8PK/ by russau
...
function getOtp(key) {
    ...
    //logger.info(time);
    //logger.info(base32tohex(key));

    var shaObj = new jsSHA("SHA-1", "HEX");
    shaObj.setHMACKey(base32tohex(key), "HEX");
    shaObj.update(time);
    var hmac = shaObj.getHMAC("HEX");
    //logger.info(hmac);
    //logger.info("HMAC SUB2: "+hmac.substr(offset * 2, 8));
